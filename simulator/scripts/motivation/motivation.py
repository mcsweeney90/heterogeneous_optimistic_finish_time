...
def MCS(dag, platform, comp_sample, comm_sample, production_steps=10, selection_steps=10, heuristic="heft", threshold=0.1):
    """ 
    Monte Carlo Scheduling heuristic.
...
# Load the data. 
# TODO: Ugly code, tidy up (works so not a priority atm). 
...
#                        task.comm_costs["GC"] = gc_comm_costs
#                        task.comm_costs["GG"] = gg_comm_costs
#                    # TODO: ACCIDENTALLY SAVED ALL MAKESPANS INSTEAD OF JUST THE MEAN! FIX.
...
#               
#                print("Number of runs: {}".format(runs), file=dest)   TODO: ERROR IN HOW MEAN IS CALCULATED, SAME AS ABOVE.             
#                print("Mean dynamic makespan: {}".format(np.mean(dynamic_mkspans[env.name][nb])), file=dest)
#                print("Mean static makespan: {}".format(np.mean(static_mkspans[env.name][nb])), file=dest)
#                print("Mean mixed makespan: {}".format(np.mean(mixed_mkspans[env.name][nb])), file=dest)  
#                print("Mean MCS makespan: {}".format(np.mean(mcs_mkspans[env.name][nb])), file=dest)                   
#                print("--------------------------------------------------------\n", file=dest)  
