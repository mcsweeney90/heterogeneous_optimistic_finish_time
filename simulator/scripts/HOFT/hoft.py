...
n_tasks = [35, 220, 680, 1540, 2925, 4960, 7770, 11480, 16215, 22100]
preferences = {"HEFT-WM" : [":", "o"], "HOFT" : ["--", "s"], "HOFT-WM" : ["-.", "^"]}
for nb in [128, 1024]:
    ...
    plt.xscale('log')
    for h in heuristics:
        # Comment out next two lines to include HOFT-WM in plot (not shown in paper).
        if h == "HOFT-WM":
            continue
    ...
    for env in [single, multiple]:  
        ...
        for acc in ["low_acc", "high_acc"]:
            ...
            for h in heuristics:
                ...
                print("Mean speedup: {}".format(np.mean(all_speedups[env.name][acc][h])), file=dest)
                print("Best speedup: {}".format(max(all_speedups[env.name][acc][h])), file=dest)
                print("Worst speedup: {}".format(min(all_speedups[env.name][acc][h])), file=dest)
                times_better_than_HEFT = sum(s > 0 for s in all_speedups[env.name][acc][h])
                print("Number of times better than HEFT: {}/{}".format(times_better_than_HEFT, len(all_speedups[env.name][acc][h])), file=dest)
        print("\n\n\n\n\n", file=dest)      
